...
def load_model():
    with open('modelo_03_prophet.json', 'r') as file_in:
...
tabela_previsao['Data (Dia/Mês/Ano)'].dt.strftime('%d-%m-%Y')
tabela_previsao['O3 (ug/m3)'] = tabela_previsao['O3 (ug/m3)'].round(2)
tabela_previsao.reset_index(drop=True, inplace=True)
st.write(f'Tabela contendo as previsões de ozônio (ug/m3) paras os próximos {dias} dias')
